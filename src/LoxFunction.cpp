...
LoxFunction::LoxFunction(const FunctionStmt *declaration, std::shared_ptr<Environment>closure, bool isInitializer)
    : m_declaration(declaration), m_closureptr(closure), m_isInitializer(isInitializer) {}
...
LiteralValue LoxFunction::call(Interpreter &interpreter,
    ...
    try {
        ...
        if (m_isInitializer) {
            return m_closureptr->getAt(0, Token(TokenType::THIS, "this", m_declaration->name.line));
        }
        return e.getValue();
    }
