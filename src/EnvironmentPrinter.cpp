...
struct LiteralPrinter {
    ...
    std::string operator()(double d) const {
        ...
        if (s == "-") s = "-0";
        if (s.empty() && std::isinf(d)) return d > 0 ? "inf" : "-inf";
        if (s.empty() && std::isnan(d)) return "nan";
        if (s.empty()) return "0";
        return s;
    }
...
void formatEnvironmentRecursive(std::stringstream& ss, const Environment* env, size_t depth) {
    if (!env) return;

    // Define exact field widths matching the image
    const int nameFieldWidth = 26; // Adjusted from 25
    const int valueFieldWidth = 35;
    const int innerWidth = nameFieldWidth + 1 + valueFieldWidth; // 26 + 1 + 35 = 62
    const int totalWidth = 1 + innerWidth + 1; // 1 + 62 + 1 = 64

    // Top border: +--------------------------+-----------------------------------+
    ss << "+" << std::string(nameFieldWidth, '-') << "+" << std::string(valueFieldWidth, '-') << "+\n";

    // Scope header: |            SCOPE 0 (0xaddr)           |
    std::stringstream scope_ss;
    // Use std::hex to format the pointer address
    scope_ss << " SCOPE " << depth << " (0x" << std::hex << reinterpret_cast<uintptr_t>(env) << ") ";
    ...
    ss << "|" << std::string(scopePaddingLeft, ' ') << scopeText << std::string(scopePaddingRight, ' ') << "|\n";
    ...
    if (env->m_values.empty()) {
        ...
        ss << "|" << std::string(emptyPaddingLeft, ' ') << emptyMsg << std::string(emptyPaddingRight, ' ') << "|\n";
    } else {
